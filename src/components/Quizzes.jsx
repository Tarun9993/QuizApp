...
const Quizzes = () => {
  ...
  return (
    ...
    <section className='max-w-7xl mx-auto py-6'>
        <div className=''>
            <h1 className={` ${isDark ? "text-white" : ""} font-bold text-2xl tracking-wider py-1`}>Available Quizzes</h1>
            ...
            {quizzes.map((item) => {
                return(
                    <div key={item.id} className={` ${isDark ? "bg-[#141F38]" : "bg-[#FFFFFF]"} rounded-xl space-y-1 max-w-[26rem] p-6  shadow-xl hover:scale-105 transform transition duration-300 ease-in-out`}>
                        ...
                        <h1 className={`${isDark ? "text-white" : "text-neutral-600"} font-bold text-xl p-3 tracking-wide`}>{item.quizTitle}</h1>
